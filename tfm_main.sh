...
if [ "x$clustertype" != "xclean" ]; then
   ...
   if [ "x$clustertype" == "xlda" ]; then
      ...
      $MAHOUT seq2sparse \
         ...
         -o ${WORK_DIR}/tfm-out-seqdir-sparse-lda -ow --maxDFPercent 85 --namedVector \
      && \
      $MAHOUT rowid \
         -i ${WORK_DIR}/tfm-out-seqdir-sparse-lda/tfidf-vectors \
